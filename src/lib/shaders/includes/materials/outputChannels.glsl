...
void writeOutputChannels( const in OutputChannels outputChannels, const int outputChannelSelector ) {
  ...
  else if( outputChannelSelector == OUTPUT_CHANNEL_METALNESS_ROUGHNESS_OCCLUSION ) {
    result.r = outputChannels.metalness;
    result.g = outputChannels.roughness;
    result.b = outputChannels.occlusion;
  }
