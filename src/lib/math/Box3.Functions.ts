...
export function makeBox3FromSphereBounds(box: Box3, sphere: Sphere): Box3 {
if (sphere.isEmpty()) {
...
box.set(sphere.center, sphere.center);
box.expandByScalar(sphere.radius);
...
export function box3ClampPoint( box: Box3, point: Vector3, result: Vector3 ): Vector3 {
  return result.copy(point).clamp(box.min, box.max);
}

export function box3DistanceToPoint( box: Box3, point: Vector3): number {
  // TODO: Remove memory allocation
  return point.clone().clamp(this.min, this.max).sub(point).length();
}

export function box3Box3Intersect(a: Box3, box) {
