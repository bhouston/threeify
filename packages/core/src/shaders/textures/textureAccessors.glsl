...
vec4 sampleTexture(
  const TextureAccessor textureAccessor,
  const vec2 uvs[NUM_UV_CHANNELS]
) {
  return texture(
    textureAccessor.texture,
    mat3TransformUV(textureAccessor.uvTransform, uvs[textureAccessor.uvIndex])
  );
}
...
float sampleTextureComponent(
  const ComponentTextureAccessor textureAccessor,
  const vec2 uvs[NUM_UV_CHANNELS]
) {
  return texture(
    textureAccessor.texture,
    mat3TransformUV(textureAccessor.uvTransform, uvs[textureAccessor.uvIndex])
  )[textureAccessor.textureComponent];
}
