...
bool ellipseRayIntersection(Ray ray, Ellipse ellipse, out vec3 hitPosition, out vec3 hitNormal) {
    ...
    vec3 hitPositionInEllipseSpace = (hitPosition - ellipse.origin) / ellipse.radii;
    hitNormal = normalize(hitPositionInEllipseSpace);
    hitNormal = normalize(vec3(hitNormal.x / ellipse.radii.x, hitNormal.y / ellipse.radii.y, hitNormal.z / ellipse.radii.z));
