...
vec3 getVolumeAttenuation( const in float distance, const in vec3 attenuationColor, const in float attenuationDistance ) {
    ...
    if ( isinf( attenuationDistance ) ) return 1.;
...
vec4 BTDF_TransmissionAttenuation(
    const in vec3 worldNormal, const in vec3 worldViewDirection, const in vec3 worldPosition,
    const in mat4 localToWorld, const in mat4 worldToView, const in mat4 viewToScreen,
    const in vec3 albedo, const in vec3 specularF0, const in float specularF90,
    const in float ior, const in float specularRoughness, const in float localThickness,
    const in vec3 attenuationColor, const in float attenuationDistance,
     onst in sample2D backgroundTexture ) {
