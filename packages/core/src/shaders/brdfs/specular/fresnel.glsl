...
vec3 F_Schlick_RoughnessDependent( const vec3 F0, const float NdotV, const float roughness ) {
  // See F_Schlick
...
// validated from KHR_material_ior, https://github.com/KhronosGroup/glTF/blob/main/extensions/2.0/Khronos/KHR_materials_ior/README.md
float iorToF0( const float ior ) {
    return pow2( ( ior - 1.0 ) / ( ior + 1.0 ) );
}

// validated from KHR_material_specular, https://github.com/KhronosGroup/glTF/blob/main/extensions/2.0/Khronos/KHR_materials_specular/README.md
vec3 fresnelMix( const vec3 f0, const vec3 f90, const float VdotH, const float weight, const vec3 base, const vec3 layer ) {
