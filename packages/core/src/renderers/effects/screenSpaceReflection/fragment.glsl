...
vec2 getScreenSpaceReflection(
  vec3 viewSpacePosition,
  vec3 viewSpaceNormal,
  vec3 viewDirection
) {
  vec2 screenSpaceRay = getViewSpaceRay(
    viewSpacePosition,
    viewSpaceNormal,
    viewDirection
  );
  ...
  for (int i = 0; i < MAX_STEPS; i++) {
    ...
    if (
      screenSpaceTexCoord.x < 0.0 ||
      screenSpaceTexCoord.x > 1.0 ||
      screenSpaceTexCoord.y < 0.0 ||
      screenSpaceTexCoord.y > 1.0 ||
      rayLength > MAX_REFLECTION_DISTANCE
    ) {
      break;
    }

    float depth = texture(uDepth, screenSpaceTexCoord).r;
    vec3 hitViewSpacePosition = getViewSpacePosition(
      screenSpaceTexCoord,
      depth
    );
...
void main() {
  ...
  vec2 reflectionTexCoord = getScreenSpaceReflection(
    viewSpacePosition,
    viewSpaceNormal,
    viewDirection
  );
