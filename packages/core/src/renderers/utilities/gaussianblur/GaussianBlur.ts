...
import { renderBufferGeometry, renderPass } from '../../webgl/framebuffers/VirtualFramebuffer';
...
export class GaussianBlur implements IDisposable {
  ...
  async exec(
    ...
    const uniforms = {
      ...
      kernelRadiusInTexels: Math.ceil(standardDeviationInTexels * 2.5),
    ...
    using(
      ...
      (targetFramebuffer) => {
        renderPass({
          ...
          program,
          uniforms,
          blendState: ( targetAlpha < 1.0 ) ? BlendState.PremultipliedOver : BlendState.None,
        });
      }
    );
  }
