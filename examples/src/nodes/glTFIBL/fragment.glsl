...
void main( ) {
  ...
  if( iblMapIntensity != vec3( 0. ) ) {
   ...
   // vec3 reflectDirection = reflect( -viewDirection, viewNormal );
    ...
    float clearCoatDotVN = saturate( dot( viewViewDirection, viewClearcoatNormal ) );

    vec3 iblIrradiance = sampleIBLIrradiance( iblMapTexture, iblMapIntensity, iblMapMaxLod, viewNormal, worldToView );
    vec3 iblRadiance = sampleIBLRadiance( iblMapTexture, iblMapIntensity, iblMapMaxLod, viewNormal, viewViewDirection, worldToView, material.specularRoughness );
    vec3 iblClearcoatRadiance = sampleIBLRadiance( iblMapTexture, iblMapIntensity, iblMapMaxLod, viewClearcoatNormal, viewViewDirection, worldToView, material.clearcoatRoughness );
    ...
    BRDF_Specular_GGX_Multiscatter_IBL( viewNormal, viewViewDirection, specularF0, specularF90, material.specularRoughness, singleScattering, multiScattering );
    ...
    indirect_brdf = indirect_brdf * ( 1. - max3( singleScattering + multiScattering ) ) + specular_brdf * specOcclusion;

    // sheen
    if( material.sheenColor != vec3( 0. ) ) {
      vec3 sheen_brdf = iblIrradiance * BRDF_Sheen_Charlie_IBL( viewNormal, viewViewDirection, material.sheenColor, material.sheenRoughness ) * material.occlusion;
      indirect_brdf = sheenMix( material.sheenColor, indirect_brdf, sheen_brdf );
    }

    if( material.clearcoatFactor != 0. ) {
      vec3 clearcoatHalfDirection = normalize( viewClearcoatNormal + viewViewDirection );
      float clearcoatVdotH = saturate( dot( viewViewDirection, clearcoatHalfDirection ) );

    // clearcoat
      vec3 clearcoat_brdf = iblClearcoatRadiance * BRDF_Specular_GGX_IBL( viewClearcoatNormal, viewViewDirection, clearcoatF0, clearcoatF90, material.clearcoatRoughness ) *
        specularOcclusion( clearCoatDotVN, material.occlusion, material.clearcoatRoughness );
      indirect_brdf = fresnelMix( clearcoatF0, clearcoatF90, clearcoatVdotH, material.clearcoatFactor, indirect_brdf, clearcoat_brdf );
    }
  ...
  for( int i = 0; i < MAX_PUNCTUAL_LIGHTS; i++ ) {
   ...
   // }
    ...
    vec3 specular_brdf = irradiance * BRDF_Specular_GGX_NoFrenel( viewNormal, viewViewDirection, directLight.direction, material.specularRoughness ) *
      specularOcclusion( dotNV, material.occlusion, material.specularRoughness );
    direct_brdf = fresnelMix( specularF0, specularF90, VdotH, material.specularFactor, diffuse_brdf, specular_brdf );

    // sheen
    if( material.sheenColor != vec3( 0. ) ) {
      vec3 sheen_brdf = irradiance * BRDF_Sheen_Charlie( viewNormal, viewViewDirection, directLight.direction, material.sheenColor, material.sheenRoughness ) * material.occlusion;
    ...
    if( material.clearcoatFactor != 0. ) {
      vec3 clearcoat_brdf = clearcoatIrradiance *
        BRDF_Specular_GGX_NoFrenel( viewClearcoatNormal, viewViewDirection, directLight.direction, material.clearcoatRoughness ) *
        specularOcclusion( clearCoatDotVN, material.occlusion, material.clearcoatRoughness );
      direct_brdf = fresnelMix( clearcoatF0, clearcoatF90, VdotH, material.clearcoatFactor, direct_brdf, clearcoat_brdf );
    }
